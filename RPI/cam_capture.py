...
def init_camera(resolution="low"):
    ...
    logging.info(
        f"Starting camera, resolution set to: {selected_res[0]} x {selected_res[1]}"
    )
...
def capture_images(
    picam2, interval=0.2
):  # 0.2 second interval, approx 5 images per second
    while True:
        try:
...
def change_resolution(resolution):
    ...
    if resolution in ["low", "medium", "high", "max"]:
        ...
        capture_thread = threading.Thread(
            target=capture_images, args=(picam2,), daemon=True
        )
