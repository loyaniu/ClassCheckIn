...
export default function App() {
  ...
  const addManualCheckin = useMutation(api.checkins.manualCreate);
    ...
    if (timeFilter !== TIME_FILTERS.ALL) {
      ...
      switch (timeFilter) {
        ...
        case TIME_FILTERS.DAYS_7:
          ...
          break;
        default:
          // Should not happen, but return all if it does
          timeThreshold = 0;
      }
      timeFiltered = checkins.filter(checkin => checkin.timestamp >= timeThreshold);
    }

    // Return filtered and reversed checkins
    return timeFiltered;
  };
  ...
  const handleManualCheckin = async (e: React.FormEvent) => {
    ...
    try {
      await addManualCheckin({ name: manualName, email: manualEmail });
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen p-4 gap-6 bg-gray-50">
      {/* Check-in data panel */}
      <div className="flex-1 bg-white rounded-xl p-4 shadow-sm overflow-hidden md:max-h-[calc(100vh-32px)] flex flex-col">
        ...
        <form
          ...
          className="mb-4 p-4 border border-gray-200 rounded-lg bg-gray-50 flex-shrink-0"
        >
          <h3 className="text-md font-semibold text-gray-700 mb-2">Manual Check-in</h3>
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 block w-full py-1.5 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
              required
            />
            <input
              type="text"
            ...
            <button
              type="submit"
              className="px-4 py-1.5 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors text-sm flex items-center justify-center sm:w-auto"
            >
              <svg
                className="w-4 h-4 mr-1"
                fill="none"
        ...
        <div className="overflow-y-auto flex-grow pr-2 min-h-[200px]">
          {checkins ? (
            filteredCheckins().length > 0 ? (
              ...
              <div className="text-center py-10 text-gray-500">
                <p>
                  No check-ins found
                  {timeFilter !== TIME_FILTERS.ALL ? " in selected time range" : ""}
                </p>
              </div>
            )
          ) : (
      ...
      <div className="flex-1 bg-white rounded-xl p-4 shadow-sm mt-4 md:mt-0 flex flex-col items-center overflow-hidden md:max-h-[calc(100vh-32px)]">
        ...
        <div className="flex-1 w-full min-h-[200px] overflow-hidden rounded-lg bg-gray-100 flex items-center justify-center">
          <img
        ...
        <p className="mt-3 text-gray-500 text-xs text-center flex-shrink-0">
